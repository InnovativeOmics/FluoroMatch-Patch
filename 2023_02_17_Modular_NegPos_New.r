...
#2 Make sure you have 'Neg' at the end of a negative mode file, and 'Pos' at the end of a positive mode file.
#3 If you have AIF data, the .ms1 and .ms2 files must have the same file name
# Example:
# InputFolder
# --Shrimp
# ----shrimpFeatureTable_Neg.csv
# ----featureTableshrimp_Pos.csv
# ----Shrimp14_AIF_Pos.ms1
# ----Shrimp14_AIF_Pos.ms2
# ----Shrimp14_ddMS_Pos.ms2
# ----Shrimp14_ddMS_Neg.ms2
# ----.
# ----.
# ----.
# --Cricket
# ----featureTableCricket_Neg.csv
# ----featureTableCricket_Pos.csv
# ----Cricket2_AIF_Neg.ms1
# ----Cricket2_AIF_Neg.ms2
# ----Cricket14_ddMS_Pos.ms2
# ----Cricket22_ddMS_Neg.ms2
# ----.
# ----.
# ----.
#4 Do not worry about creating an output folder. It will auto create one.
...
if (ManuallyInputVariables==TRUE){
  ...
  #parametersDirectory
  parametersDir <- "C:/FluoroMatch-3.0/LipidMatch-4.2/Flow/LipidMatch_Distribution/"
...
arguments = list(
  ...
  ,path_to_output_folder = OutputDirectory #CHECK
  # ,fn_csv_output = "EXAMPLE_CSV_OUTPUT.csv" # this is converted from mzxml data
  ,fn_MS1_output = "EXAMPLE_MS1_OUTPUT.csv"
  ,fn_eic_output = "EXAMPLE_EIC_OUTPUT.csv" # this is where the EIC is stored using feature table
  ,mztol = PrecursorMassAccuracy/2
  ,rttol = (RT_Window/2) * 6
  ,mzZoomWindow = 5
  # ,FeatureID_Cols = c(7,8,13) #mz, rt, rowID
  ,FeatureID_Cols = c(7,8,1,5) #mz, rt, rowID, formula
  # ,FeatureID_Cols = c(1,2,3) #mz, rt, rowID
  # ,FeatureID_SkipRows = c(1) #c() means don't skip, c(1) means skip the first row
  ...
  ,UseAgg = TRUE
  ,IsoThreshold = PrecursorMassAccuracy/2
)
#
...
suppressWarnings(if (runNegddMS) {
  arguments$fn_FeatureID<-"NegIDed_FIN.csv"
  arguments$fn_eic_output<-"Neg_Feature_EICs.csv"
  arguments$fn_MS1_output<-"Neg_Feature_MS1s.csv"
  NegTarget_mzXML<-grep(Target_mzXML, pattern = "_Neg|_neg|_NEG", invert=FALSE, value=TRUE)
  for (i in 1:length(NegTarget_mzXML)) {
    arguments$fn_mzxml = NegTarget_mzXML[i] #CHECK
    extract_EICs(arguments)
    extract_MS1(arguments)
  }
})

suppressWarnings(if (runPosddMS) {
  arguments$fn_FeatureID<-"PosIDed_FIN.csv"
  arguments$fn_eic_output<-"Pos_Feature_EICs.csv"
  arguments$fn_MS1_output<-"Pos_Feature_MS1s.csv"
  PosTarget_mzXML<-grep(Target_mzXML, pattern = "_Pos|_pos|_POS", invert=FALSE, value=TRUE)
  for (i in 1:length(PosTarget_mzXML)) {
    arguments$fn_mzxml = PosTarget_mzXML[i] #CHECK
    extract_EICs(arguments)
    extract_MS1(arguments)
  }
})